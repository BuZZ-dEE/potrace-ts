...
export class Potrace {
  ...
  _bmToPathlist(): void {
    ...
    function findPath(point: Point): Path & {sign?: string} {
      ...
      for (;;) {
        ...
        const l = blackMap.getValueAt(
          x + (dirx + diry - 1) / 2,
          y + (diry - dirx - 1) / 2,
        );
        const r = blackMap.getValueAt(
          x + (dirx - diry - 1) / 2,
          y + (diry + dirx - 1) / 2,
        );
  ...
  _processPath(): void {
    ...
    function calcSums(
      path: Path & {x0?: number; y0?: number; sums?: Array<Sum>},
    ): void {
      let i, x, y;
      path.x0 = path.pt[0].x;
      path.y0 = path.pt[0].y;
    ...
    function bestPolygon(
      path: Path & {lon?: Array<number>; m?: number; po?: Array<number>},
    ): void {
      function penalty3(
        path: Path & {sums?: Array<Sum>},
        i: number,
        j: number,
      ): number {
        const n = path.len,
    ...
    function adjustVertices(
      path: Path & {m?: number; po?: Array<number>; x0?: number; y0?: number},
    ): void {
      function pointslope(
        path: Path & {sums?: Array<Sum>},
        i: number,
    ...
    function optiCurve(path: Path): void {
      function opti_penalty(
        ...
        const m = path.curve.n,
          curve = path.curve,
          vertex = curve.vertex;
        let k, k1, k2, area, d, d1, d2, p1, p2, pt, t;
  ...
  _validateParameters(params: PotraceOptions): void {
    if (
      params &&
      params.turnPolicy &&
      Potrace.SUPPORTED_TURNPOLICY_VALUES.indexOf(params.turnPolicy) === -1
    ) {
      const goodVals =
        "'" + Potrace.SUPPORTED_TURNPOLICY_VALUES.join("', '") + "'";
  ...
  setParameters(newParams: PotraceOptions): void {
    ...
    const thisParams: Record<keyof _PotraceOptions, PotraceParameterValue> =
      this._params;
