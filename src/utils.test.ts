...
describe("utils", () => {
    it("adds, replaces, and caches HTML attributes", () => {
        expect(utils.setHtmlAttribute("<svg></svg>", "width", "10")).toBe('<svg width="10"></svg>');
        expect(utils.setHtmlAttribute('<svg width="10"></svg>', "width", "20")).toBe('<svg width="20"></svg>');
    ...
    it("evaluates Bezier curves and tangents", () => {
        ...
        expect(utils.tangent(p0, p1, p2, p3, new Point(0, 0), new Point(1, 0))).toBeGreaterThanOrEqual(0);
        expect(utils.tangent(p0, p0, p0, p0, new Point(0, 0), new Point(1, 0))).toBe(-1);
    });
});
